/// The syscalls behind the paste tier: a promised pasteboard item and a ⌘V
/// chord. The *decisions* — when a receipt counts, when to settle — are in
/// `HelperCore/PasteTransaction.swift`, which is pure and unit-tested.
///
/// **Nothing here ever reads the pasteboard.** Contract §5: the pasteboard is
/// written, never read. Reading is the operation macOS 15.4 previewed a prompt
/// for and macOS 26 carries; writing never prompts. A design that only writes is
/// immune to that permanently, which is why there is no snapshot and no restore
/// anywhere in this file.
///
/// The mechanism is Handy's (`src-tauri/src/paste_tx/`, MIT). Read, understood,
/// and reimplemented against this codebase — no source copied, and the split
/// between a pure decision and a thin syscall binding is ours rather than
/// theirs. The idea deserves the attribution: *do not put data on the
/// pasteboard, put a promise on it and let macOS tell you when someone reads
/// it.*

import AppKit
import ApplicationServices
import CoreGraphics
import Foundation
import HelperCore

/// A pasteboard publication with no data behind it.
///
/// `declareTypes:owner:` announces which types are available; AppKit calls back
/// on `pasteboard:provideDataForType:` only when a consumer actually asks for
/// one. **That callback is a read receipt** — the only native signal on macOS
/// that says "the target took our text", and a strictly stronger one than
/// either verifier this repo used to ship, because it works in applications
/// whose AX text length is permanently zero.
///
/// **Lifetime.** AppKit holds the owner *weakly* (`declareTypes(_:owner:)` takes
/// `Any?` and does not retain it). If this object is deallocated before the
/// target reads, the promise resolves to nothing and the user pastes an empty
/// string. Whoever publishes must hold a strong reference until the transaction
/// settles — `PasteInserter` does, on the stack of the insertion it belongs to.
///
/// **Threading.** Both callbacks arrive on the run loop that was current when
/// `publish` was called, which for this process must be the main one (see the
/// note at the top of `HelperApp.swift`). They are handed straight to the
/// closures; the closures are responsible for being main-thread-safe.
final class PromisedPasteboardOwner: NSObject, NSPasteboardTypeOwner {
    /// The types this publication declares are `PasteboardPromise`'s, which is
    /// where the convention and the reasoning behind each of them live.
    /// Ours, carrying a per-publication UUID.
    ///
    /// **Written, never read** — by us or by anything else. It exists so that a
    /// clipboard manager, a `pbpaste -Prefer`, or a person reading a bug report
    /// can tell a Grok Dictate publication apart from a user's own ⌘C. Nothing
    /// in the settle path consults it: ownership is decided by `changeCount`
    /// and `pasteboardChangedOwner:`, both of which are answers from the OS,
    /// and a UUID we would have to *read back* to use would reintroduce exactly
    /// the pasteboard read this design exists to avoid.
    static let sessionType = NSPasteboard.PasteboardType(PasteboardPromise.sessionType)

    /// Built from `PasteboardPromise` rather than from literals here, so that
    /// `ClipboardDisciplineTests` can assert the list — and the absence of
    /// `ConcealedType` from it — without a windowserver.
    static let declaredTypes = PasteboardPromise.declaredTypes.map(
        NSPasteboard.PasteboardType.init(rawValue:))

    /// What a `provideDataForType:` callback means, classified here so that
    /// nothing above this file has to know an `NSPasteboard.PasteboardType`.
    ///
    /// The distinction is the difference between a receipt and a rumour. A
    /// clipboard manager reacting to the pasteboard change asks for the marker
    /// types, because that is how it decides whether to record the entry; only
    /// a request for the *text* means someone is about to put our transcript
    /// somewhere.
    enum Request: Equatable {
        /// A consumer asked for the transcript. **This is the read receipt.**
        case text
        /// Something inspected one of our markers. Not a receipt.
        case marker(String)
    }

    /// The general pasteboard's current change count.
    ///
    /// The ownership token, and the only thing this design ever reads from the
    /// pasteboard — a counter, not content. It does not trip the privacy
    /// machinery in macOS 15.4+, which gates access to pasteboard *data*.
    static var currentChangeCount: Int { NSPasteboard.general.changeCount }

    private let text: String
    private let sessionId = UUID().uuidString
    private let shouldProvideText: () -> Bool
    private let onRequest: (Request) -> Void
    private let onOwnershipLost: () -> Void

    init(
        text: String,
        shouldProvideText: @escaping () -> Bool = { true },
        onRequest: @escaping (Request) -> Void,
        onOwnershipLost: @escaping () -> Void
    ) {
        self.text = text
        self.shouldProvideText = shouldProvideText
        self.onRequest = onRequest
        self.onOwnershipLost = onOwnershipLost
    }

    /// Publish the promise. Returns the pasteboard's new `changeCount`, which is
    /// the token the settle path checks ownership against.
    @discardableResult
    func publish(to pasteboard: NSPasteboard = .general) -> Int {
        pasteboard.declareTypes(Self.declaredTypes, owner: self)
    }

    /// Settle: take the transcript back off the pasteboard.
    ///
    /// `clearContents()` and nothing else — no restore, because restoring would
    /// mean having read the previous contents. The user's earlier clipboard is
    /// gone; that trade is stated in `README.md` and in the settings copy.
    ///
    /// Returns `false` when somebody else has taken the pasteboard since
    /// `expectedChangeCount`, in which case nothing is touched: their copy wins.
    @discardableResult
    func clear(from pasteboard: NSPasteboard = .general, ifChangeCountIs expected: Int) -> Bool {
        guard pasteboard.changeCount == expected else { return false }
        pasteboard.clearContents()
        return true
    }

    // MARK: - NSPasteboardTypeOwner

    @objc func pasteboard(
        _ pasteboard: NSPasteboard,
        provideDataForType type: NSPasteboard.PasteboardType
    ) {
        if type == .string || type.rawValue == PasteboardPromise.textType {
            // A clipboard manager asking before ⌘V must not receive the
            // transcript: it would cache the bytes, the later target paste
            // would never generate a receipt, and the ladder would wait,
            // clear, and inject — double-type. Provide nothing, and do not
            // record a landing.
            guard shouldProvideText() else { return }
            if pasteboard.setString(text, forType: type) {
                onRequest(.text)
            }
            return
        }
        if type == Self.sessionType {
            pasteboard.setString(sessionId, forType: type)
            onRequest(.marker(type.rawValue))
            return
        }
        if type.rawValue == PasteboardPromise.transientType
            || type.rawValue == PasteboardPromise.autoGeneratedType
        {
            // The two nspasteboard.org markers are flags, not payloads. An
            // empty string is what the convention expects and what every
            // manager that honours it reads. Not a receipt.
            pasteboard.setString("", forType: type)
            onRequest(.marker(type.rawValue))
            return
        }
        // Unknown types: provide nothing. An empty string here used to make
        // managers cache a blank entry as if we had fulfilled the promise.
    }

    @objc func pasteboardChangedOwner(_ pasteboard: NSPasteboard) {
        onOwnershipLost()
    }
}

/// The transaction, and the lock that lets the callbacks (main thread) and the
/// wait (insertion queue) both touch it.
///
/// **Deliberately does not reference the owner.** The owner's callbacks capture
/// this, so an owner reachable from here would be an owner that keeps itself
/// alive — one leaked promise per dictation, invisible until the process has
/// been up for a day.
private final class PasteState {
    private let lock = NSLock()
    private var transaction: PasteTransaction

    init(_ transaction: PasteTransaction) {
        self.transaction = transaction
    }

    func read<T>(_ body: (PasteTransaction) -> T) -> T {
        lock.lock()
        defer { lock.unlock() }
        return body(transaction)
    }

    func write<T>(_ body: (inout PasteTransaction) -> T) -> T {
        lock.lock()
        defer { lock.unlock() }
        return body(&transaction)
    }
}

/// One paste that has published and not yet released.
///
/// Three things have to outlive the call that started them: the owner, because
/// AppKit holds it weakly and a deallocated owner resolves the promise to
/// nothing; the state, because the release happens after the insertion has
/// already been reported; and the `changeCount`, which is the token deciding
/// whether we may still take the pasteboard back.
private final class LivePaste {
    let owner: PromisedPasteboardOwner
    let changeCount: Int
    let state: PasteState

    init(owner: PromisedPasteboardOwner, changeCount: Int, state: PasteState) {
        self.owner = owner
        self.changeCount = changeCount
        self.state = state
    }
}

/// Tier 1 on the paste route.
///
/// **Threading, and getting it wrong is the canonical dead-hotkey bug.** This
/// process's main thread runs the `CFRunLoop` that owns the `CGEventTap`, and
/// stalling it gets the tap disabled by macOS with
/// `kCGEventTapDisabledByTimeout`. Insertion therefore runs on
/// `BackgroundInsertion`'s serial queue. But promised pasteboard data is
/// serviced by AppKit **on the main run loop**, so the promise has to be
/// published there or nothing will ever answer a consumer's request. The split:
///
///   - **publish on main**, briefly and synchronously;
///   - **wait the observer grace on the insertion queue**, so a clipboard
///     manager's change-notification read happens before `chordAt`;
///   - **post the chord on main**;
///   - **wait on the insertion queue**, where sleeping is free;
///   - **release back on main**, after the insertion has already been reported.
///
/// The release is deliberately not part of the wait. Reporting happens on the
/// first receipt — the moment the target has the text — and the pasteboard is
/// taken back later, once the reads go quiet. Folding the two together would
/// put `PasteTransaction.receiptQuietPeriod` into every insertion's latency,
/// which is the number this whole tier exists to reduce.
final class PasteInserter: PasteInserting {
    /// Where the ⌘V chord enters the system.
    ///
    /// **Chosen, not measured**, and the measurement is written down as still
    /// open in `docs/report-insertion-2026-09-06.md` §9.5 Q1 — a chord cannot be
    /// posted from a terminal that does not hold Accessibility, and this
    /// machine's does not.
    ///
    /// The HID tap is where Espanso, cliclick and Karabiner put their events and
    /// reaches the widest set of applications. The alternative,
    /// `CGEvent.postToPid`, is what the injection tier prefers — but FluidVoice
    /// forces the *global* clipboard path for Ghostty, which is evidence that
    /// pid-posted ⌘V is unreliable in exactly the Electron terminals that take
    /// 59 % of this app's traffic.
    ///
    /// The cost of being wrong is bounded and visible: no receipt, so the ladder
    /// falls through to injection and the user gets their text at the old speed.
    /// `--probe-paste --route pid|hid` settles it in two runs.
    private static let chordRoute: PasteChord.Route = .hidTap

    /// How often the insertion queue asks whether the paste has resolved.
    ///
    /// Adds up to this much to a landed paste, against a chord-to-receipt time
    /// measured in tens of milliseconds. Small enough not to matter, large
    /// enough not to spin.
    private static let waitPollInterval: TimeInterval = 0.01

    /// How often main asks whether the pasteboard may be taken back. Coarser
    /// than the wait because the quiet period is 200 ms and this runs on the
    /// thread that owns the event tap.
    private static let releasePollInterval: TimeInterval = 0.05

    private let settings: Settings
    private let log: (LogLevel, String) -> Void

    /// The paste that has published and not yet released. **Main thread only.**
    /// Held so the owner survives, and so `settleNow()` can take the pasteboard
    /// back if the helper is asked to quit mid-transaction.
    private var live: LivePaste?

    init(settings: Settings, log: @escaping (LogLevel, String) -> Void) {
        self.settings = settings
        self.log = log
    }

    private static func now() -> TimeInterval { ProcessInfo.processInfo.systemUptime }

    func paste(_ text: String, into app: FrontmostAppInfo) -> TierAttempt {
        // **Checked here, before anything is published.** `CGEvent.post` from an
        // untrusted process is dropped *silently* — no error, no return value,
        // nothing. Without this the tier would publish a promise, post a chord
        // nobody receives, wait out the full 0.5 s ceiling and only then fall
        // through, on every single dictation, on a machine where the grant is
        // simply missing. That is the state a packaged build starts in, and it
        // recurs after some updates.
        guard AXIsProcessTrusted() else {
            return .failed(
                reason:
                    "Accessibility permission is not granted, so a synthetic ⌘V would be dropped "
                    + "without an error — open System Settings → Privacy & Security → "
                    + "Accessibility and enable Grok Dictate"
            )
        }

        // The chord is ⌘V and the retry hotkey is ⌃⌘V. A chord that picks up a
        // Control the user has not let go of yet is not paste — see
        // `ModifierSettle`.
        ModifierSettle.wait(timeout: settings.modifierSettleTimeout, log: log)

        // The clock starts here rather than at `declareTypes` below, so the
        // 0.5 s ceiling covers the hop to the main thread as well. Nothing else
        // reads `publishedAt`: whether a receipt counts is decided against the
        // chord.
        let state = PasteState(PasteTransaction(publishedAt: Self.now()))
        let owner = PromisedPasteboardOwner(
            text: text,
            shouldProvideText: { state.read { $0.shouldProvideText } },
            onRequest: { request in
                // A marker request is a clipboard manager reading our flags to
                // decide whether to record the entry. Only the text means
                // somebody is pasting.
                guard request == .text else { return }
                state.write { $0.recordTextReceipt(at: Self.now()) }
            },
            onOwnershipLost: { state.write { $0.recordOwnershipLost() } }
        )

        // Publish first, chord later. A clipboard manager reacting to the
        // pasteboard-change notification must be able to ask *before* `chordAt`,
        // or every such read counts as a landing and we report success for
        // text nobody pasted.
        let live = onMain { () -> LivePaste in
            let changeCount = owner.publish()
            let live = LivePaste(owner: owner, changeCount: changeCount, state: state)
            // A previous paste still waiting to release is dropped rather than
            // cleared: its `changeCount` no longer matches, so clearing would
            // take away the promise this one just published.
            self.live = live
            return live
        }

        Thread.sleep(forTimeInterval: PasteTransaction.clipboardObserverGrace)

        onMain {
            // `settleNow()` can have taken the pasteboard back during the grace
            // (shutdown). Posting ⌘V then would paste an empty clipboard.
            guard state.read({ !$0.settled }) else { return }
            if PasteChord.post(route: Self.chordRoute) {
                state.write { $0.recordChord(at: Self.now()) }
            } else {
                state.write { $0.recordChordFailure(at: Self.now()) }
                self.log(.warn, "the ⌘V chord could not be posted — no events were created")
            }
        }

        var settlement: PasteTransaction.Settlement
        while true {
            if let resolved = state.read({ $0.outcome(now: Self.now()) }) {
                settlement = resolved
                break
            }
            Thread.sleep(forTimeInterval: Self.waitPollInterval)
        }

        let target = app.name ?? app.bundleId ?? "the frontmost application"

        // Back on main for the verdict. GCD `main.sync` is not a drain of
        // AppKit pasteboard runloop sources, so a `provideDataForType:` that
        // was already queued would otherwise be dropped by `markSettled` on
        // the fail path and we would inject over a paste that did land.
        let verdict = onMain { () -> PasteTransaction.Settlement in
            _ = CFRunLoopRunInMode(CFRunLoopMode.defaultMode, 0, true)
            let verdict = state.read { $0.verdict(after: settlement) }
            if verdict == .landed {
                self.scheduleRelease(live)
            } else {
                // **Take the pasteboard back before the ladder injects.** A
                // target that reads after this point reads an empty pasteboard,
                // which is what stops the transcript arriving twice. Settled
                // is marked *before* `clearContents()`, because our own clear
                // re-enters `pasteboardChangedOwner:`.
                self.release(live)
            }
            return verdict
        }

        switch verdict {
        case .landed:
            log(.info, "pasted \(text.utf16.count) UTF-16 units — \(receiptSummary(state, target))")
            return .confirmed

        case .ownershipLost:
            return .failed(
                reason: "something else took the pasteboard before \(target) read it")
        case .abandoned:
            return .aborted(reason: "the helper was asked to quit before \(target) read it")
        case .chordFailed:
            return .failed(reason: "the ⌘V chord could not be posted")
        case .timedOut:
            return .failed(
                reason:
                    "\(target) never read the pasteboard after ⌘V — it may not paste with that key"
            )
        }
    }

    /// What the receipts said, for the log.
    ///
    /// Counts and milliseconds only. **Never what a receipt returned** — that is
    /// the transcript, and the log must not become the second keylogger the
    /// history file already is.
    private func receiptSummary(_ state: PasteState, _ target: String) -> String {
        state.read { transaction in
            let latency = transaction.chordAt.flatMap { chord in
                transaction.lastReceiptAt.map { Int(($0 - chord) * 1000) }
            }
            let when = latency.map { " \($0) ms after the chord" } ?? ""
            let reads =
                transaction.receiptsAfterChord > 1 ? " (\(transaction.receiptsAfterChord) reads)" : ""
            let early =
                transaction.receiptsBeforeChord > 0
                ? "; \(transaction.receiptsBeforeChord) earlier read(s) ignored — a clipboard "
                    + "manager reacting to the change, not the paste"
                : ""
            return "\(target) read the pasteboard\(when)\(reads)\(early)"
        }
    }

    /// Take the pasteboard back now, whatever the transaction thinks.
    ///
    /// Called from `HelperApp.shutdown`. A quit between the receipt and the
    /// quiet period would otherwise leave the transcript on the clipboard for
    /// good, which is one of the three ways this change can make things worse.
    ///
    /// A *crash* needs no equivalent: the transcript is never resident on the
    /// pasteboard, only promised, and a promise whose owner process is gone
    /// resolves to nothing. That is a property of the mechanism rather than
    /// something we arranged, and it is worth knowing.
    func settleNow() {
        guard let live else { return }
        release(live)
    }

    // MARK: - Releasing, main thread only

    private func scheduleRelease(_ live: LivePaste) {
        DispatchQueue.main.asyncAfter(deadline: .now() + Self.releasePollInterval) {
            [weak self] in
            guard let self, live.state.read({ !$0.settled }) else { return }
            if live.state.read({ $0.mayRelease(now: Self.now()) }) {
                self.release(live)
            } else {
                self.scheduleRelease(live)
            }
        }
    }

    private func release(_ live: LivePaste) {
        // Marked settled *before* the clear, because `clearContents()` fires
        // `pasteboardChangedOwner:` on the process that called it — our own
        // release arrives looking exactly like the user copying something else.
        guard live.state.write({ transaction -> Bool in
            guard !transaction.settled else { return false }
            transaction.markSettled()
            return true
        }) else { return }

        let cleared = live.owner.clear(ifChangeCountIs: live.changeCount)
        if self.live === live { self.live = nil }
        log(
            .info,
            cleared
                ? "took the transcript back off the pasteboard"
                : "left the pasteboard alone — something else owns it now"
        )
    }

    /// Run on the main thread and wait for the answer.
    ///
    /// Safe from the insertion queue: main is inside `CFRunLoopRun` and is never
    /// blocked on this queue, so there is nothing to deadlock against. The
    /// `isMainThread` check is there because `--probe-paste` calls the tier
    /// directly from main.
    private func onMain<T>(_ body: () -> T) -> T {
        if Thread.isMainThread { return body() }
        return DispatchQueue.main.sync(execute: body)
    }
}

/// The ⌘V chord.
enum PasteChord {
    /// The **physical** V key, `kVK_ANSI_V`.
    ///
    /// Hard-coded rather than derived from the current layout, and that is not
    /// laziness. Keyboard layouts whose name ends in `⌘` remap to QWERTY while
    /// Command is held, so resolving "the key that produces v" against the live
    /// layout yields the wrong keycode for exactly the users who need it right.
    /// VoiceInk hit this and ships `key code 9` for the same reason.
    static let virtualKeyV: CGKeyCode = 0x09

    enum Route: Equatable {
        /// Straight into one process's event queue, past every global tap.
        case pid(pid_t)
        /// The HID level, where Espanso, cliclick and Karabiner put their
        /// events. Reaches the widest set of applications.
        case hidTap
    }

    /// Left Command, `kVK_Command`. Posted as its own down/up around V so apps
    /// and IMEs that watch the modifier events themselves see a real chord, not
    /// a V that merely claims Command is held.
    static let virtualKeyCommand: CGKeyCode = 0x37

    /// Post ⌘V. Returns false only if the events could not be constructed —
    /// `CGEvent.post` and `postToPid` have no return channel, which is the whole
    /// reason the receipt exists.
    ///
    /// Two details are load-bearing and are the same two `UnicodeInserter`
    /// documents at length:
    ///
    /// 1. `CGEventSource(stateID: .privateState)`. A source built from
    ///    `.hidSystemState` inherits the *physically held* modifiers, and the
    ///    retry hotkey is ⌃⌘V — a chord that picks up a stray Control is not
    ///    paste, it is the retry hotkey firing again.
    /// 2. Flags set to exactly `.maskCommand` on Command-down and both V
    ///    events. `= [.maskCommand]` rather than `.insert(.maskCommand)` so
    ///    there is no state to inherit. Command-up has empty flags: the
    ///    modifier is no longer held.
    ///
    /// Callers must have waited on `ModifierSettle` first, for the half of the
    /// problem that lives in the *target's* reading of global hardware state.
    static func post(route: Route) -> Bool {
        guard
            let source = CGEventSource(stateID: .privateState),
            let commandDown = CGEvent(
                keyboardEventSource: source, virtualKey: virtualKeyCommand, keyDown: true),
            let keyDown = CGEvent(
                keyboardEventSource: source, virtualKey: virtualKeyV, keyDown: true),
            let keyUp = CGEvent(
                keyboardEventSource: source, virtualKey: virtualKeyV, keyDown: false),
            let commandUp = CGEvent(
                keyboardEventSource: source, virtualKey: virtualKeyCommand, keyDown: false)
        else { return false }

        commandDown.flags = [.maskCommand]
        keyDown.flags = [.maskCommand]
        keyUp.flags = [.maskCommand]
        commandUp.flags = []

        switch route {
        case let .pid(pid):
            commandDown.postToPid(pid)
            keyDown.postToPid(pid)
            keyUp.postToPid(pid)
            commandUp.postToPid(pid)
        case .hidTap:
            commandDown.post(tap: .cghidEventTap)
            keyDown.post(tap: .cghidEventTap)
            keyUp.post(tap: .cghidEventTap)
            commandUp.post(tap: .cghidEventTap)
        }
        return true
    }
}
